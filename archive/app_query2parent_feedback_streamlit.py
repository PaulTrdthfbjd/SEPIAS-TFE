...
with left:
    ...
    st.markdown("### Dumps crops / objets")
    ...
    top_per_parent = st.slider(
        "Nombre de crops conservés par parent",
        min_value=1,
        max_value=5,
        value=1,
    )
    ...
    show_bbox = st.checkbox(
        "Afficher la bbox sur l'image parent",
        value=True,
    )

    st.markdown("### Feedback interactif local")
    ...
    seg_method = st.selectbox(
        "Méthode de segmentation de la requête",
        ["maskrcnn", "sam_auto", "segnet", "unet"],
    )

    st.markdown("### Paramètres segmentation")
    ...
    segnet_ckpt = st.text_input(
        "Checkpoint SegNet",
        value="checkpoints/segnet_objectness_scenes.pt",
    )

    unet_ckpt = st.text_input(
        "Checkpoint U-Net",
        value="checkpoints/unet_objectness_scenes.pt",
    )
...
with right:
    ...
    if query_kind == "Crop déjà segmenté":
        ...
        try:
            if seg_method == "maskrcnn":
                ...
                query_crops = segment_query_sam_auto(
                    q_img_full,
                ...
                query_crops = segment_query_unet_or_segnet(
                    q_img_full,
                    method="segnet",
    ...
    st.subheader(f"Crops de la requête ({len(query_crops)})")

    q_cols = st.columns(5)

    for i, rec in enumerate(query_crops[:20]):
        q_cols[i % 5].image(
            to_rgb_flat(rec["crop"]),
            caption=f"qcrop #{i + 1}",
    ...
    with tab2:
        st.subheader("Résultats segmentés crop→parent")
        ...
        for j, row in enumerate(parents_ranked):
            ...
            c.image(
                parent_disp,
                caption=(
                    f"{j + 1}. score_parent={parent_score:.3f}\n"
            ...
            if enable_feedback:
                ...
                if pos and neg:
                    c.warning("Choisis soit pertinent, soit non pertinent.")
                elif pos:
                    selected_positive_rows.append(row)
                elif neg:
                    selected_negative_rows.append(row)

            with c.expander("Détails du match local"):
